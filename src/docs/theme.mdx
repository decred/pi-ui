...
## Use ThemeConext in deep children

`ThemeProvider` exposes two properties `useThemeName` and `currentTheme` which can be used in deep children using React's `useContext` hook.

```js
import React, { useContext } from "react";
import { defaultLightTheme, defaultDarkTheme, ThemeContext, Button, getThemeProperty } from "pi-ui";

const DeepChild = () => {
  const { useThemeName, currentTheme } = useContext(ThemeContext);
  const [themeMode, setThemeMode] = useThemeName;

  const colorGreen = getThemeProperty(currentTheme, "color-green"); // get theme property 
